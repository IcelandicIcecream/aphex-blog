import { l as ensure_array_like, tt as escape_html, u as head } from "../../../chunks/dev.js";
import "../../../chunks/client.js";
import { n as invitations, s as organizations } from "../../../chunks/api.js";
import { n as invalidateAll, s as resolve, t as goto } from "../../../chunks/client2.js";
import "../../../chunks/navigation.js";
import { t as Button } from "../../../chunks/button.js";
import { t as Badge } from "../../../chunks/badge.js";
import { t as authClient } from "../../../chunks/auth-client.js";
//#region src/routes/invitations/+page.svelte
	...
	$$renderer.component(($$renderer) => {
		...
		async function handleReject(invitation) {
			...
			try {
				...
				if (!result.success) throw new Error(result.error || "Failed to decline invitation");
		...
		if (pendingInvitations.length === 0) {
			...
			$$renderer.push("<!--[-1-->");
			$$renderer.push(`<div class="w-full max-w-xl space-y-4"><div class="mb-6 text-center"><h2 class="text-lg font-semibold">Pending Invitations</h2> <p class="text-muted-foreground mt-1 text-sm">You've been invited to join the following organizations</p></div> <!--[-->`);
			...
			for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
				...
				$$renderer.push(`<div class="bg-card flex items-center justify-between gap-4 rounded-lg border p-5"><div class="min-w-0 flex-1"><div class="flex items-center gap-2"><p class="truncate font-medium">${escape_html(invitation.organizationName)}</p> `);
				Badge($$renderer, {
					variant: "outline",
					class: "capitalize",
					children: ($$renderer) => {
						$$renderer.push(`<!---->${escape_html(invitation.role)}`);
					},
					$$slots: { default: true }
				});
				$$renderer.push(`<!----></div> <div class="text-muted-foreground mt-1 flex gap-3 text-xs"><span>/${escape_html(invitation.organizationSlug)}</span> <span>${escape_html(timeUntilExpiry(invitation.expiresAt))}</span></div></div> <div class="flex shrink-0 gap-2">`);
				Button($$renderer, {
					variant: "outline",
					size: "sm",
					...
					disabled: isBusy,
					children: ($$renderer) => {
						$$renderer.push(`<!---->${escape_html(isRejecting ? "Declining..." : "Decline")}`);
					},
					$$slots: { default: true }
				});
				$$renderer.push(`<!----> `);
				Button($$renderer, {
					...
					disabled: isBusy,
					children: ($$renderer) => {
						$$renderer.push(`<!---->${escape_html(isAccepting ? "Joining..." : "Accept")}`);
					},
					$$slots: { default: true }
				});
			...
			if (data.hasOrganization) {
				$$renderer.push("<!--[0-->");
				$$renderer.push(`<div class="pt-2 text-center">`);
				Button($$renderer, {
					variant: "ghost",
					onclick: () => goto("/admin"),
					children: ($$renderer) => {
