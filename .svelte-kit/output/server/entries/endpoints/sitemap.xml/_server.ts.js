...
const iso = (d) => d ? new Date(d).toISOString() : void 0;
const GET = async ({ locals, url }) => {
  ...
  try {
    ...
    const opts = { perspective: "published", limit: 1e3 };
...
${entries.map(
    (e) => `	<url><loc>${origin}${e.loc}</loc>${e.lastmod ? `<lastmod>${e.lastmod}</lastmod>` : ""}</url>`
  ).join("\n")}
</urlset>`;
