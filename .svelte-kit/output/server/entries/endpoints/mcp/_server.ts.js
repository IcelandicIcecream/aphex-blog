...
import { z as z$1, ZodOptional } from "zod";
import "hono/cors";
import "hono/bearer-auth";
import { HTTPException } from "hono/http-exception";
import { SSEStreamingApi } from "hono/streaming";
import "../../../chunks/date-utils.js";
import { a as authToContext } from "../../../chunks/auth-helpers.js";
import { v as validateSchemaReferences, R as RESERVED_FIELDS, V as VALID_FIELD_TYPES } from "../../../chunks/validator.js";
import { i as isFieldRequired, v as validateDocumentData, a as validateFile } from "../../../chunks/mime-detect.js";
...
const perspectiveArg = (v) => v === "published" ? "published" : "draft";
function collectReferences(schemaName, fields, edges, prefix = "") {
  for (const f of fields) {
    ...
    if (f.type === "reference") {
      ...
      for (const ref2 of f.of) {
        if (ref2.to && ref2.to.length > 0) {
          edges.push({ from: schemaName, path: `${path}[]`, to: ref2.to.map((t) => t.type) });
        }
      }
...
function portableTextGuide(schema) {
  const buildCustomExample = (ref2) => {
    const example = { _type: ref2.type, _key: "<unique>" };
    for (const f of ref2.fields ?? [])
      example[f.name] = `<${f.type}>`;
    return example;
  };
  const fields = {};
  for (const field of schema.fields) {
    ...
    fields[field.name] = {
      ...
      customBlockTypes: field.of.filter((o) => o.type !== "block").map((o) => ({ type: o.type, example: buildCustomExample(o) }))
    };
  }
  ...
  return [
    {
      ...
      handler: async () => {
        ...
        const objectTypes = schemas.filter((s) => s.type === "object").map((s) => ({ name: s.name, title: s.title, fieldCount: s.fields.length }));
        ...
        } : auth?.type === "session" ? { authType: "session", canWrite: true, capabilities: auth.capabilities } : { authType: "unknown", canWrite: false };
        return ok({
    ...
    },
    {
      ...
      inputSchema: {},
      handler: async () => {
        const names2 = api.getCollectionNames();
        const collections = names2.map((name) => {
    ...
    },
    {
      ...
      inputSchema: { collection: z$1.string().describe("Collection name") },
      handler: async (args) => {
    ...
    },
    {
      ...
      inputSchema: {
        collection: z$1.string().describe("Collection name"),
        data: z$1.record(z$1.string(), z$1.unknown()).describe("Document field values to validate")
      },
      handler: async (args) => {
        const collection = asString(args, "collection");
    ...
    },
    {
      ...
      inputSchema: {
        schema: z$1.record(z$1.string(), z$1.unknown()).describe("Proposed SchemaType as JSON (type, name, title, fields, …)")
      },
      handler: async (args) => {
    ...
    },
    {
      ...
      inputSchema: {
        collection: z$1.string().describe("Collection name"),
        where: z$1.record(z$1.string(), z$1.unknown()).optional().describe("Filter conditions (LocalAPI Where syntax)"),
        limit: z$1.number().optional().describe("Max results (default 50)"),
        offset: z$1.number().optional().describe("Results to skip (default 0)"),
        sort: z$1.string().optional().describe("Sort field; prefix '-' for descending, e.g. '-updatedAt'"),
        perspective: z$1.enum(["draft", "published"]).optional().describe("Which content to read (default draft)")
      },
      handler: async (args) => {
        const collection = asString(args, "collection");
        if (!collection)
        ...
        const limit2 = typeof args.limit === "number" ? args.limit : void 0;
    ...
    },
    {
      ...
      inputSchema: {
        collection: z$1.string().describe("Collection name"),
        id: z$1.string().describe("Document id"),
        perspective: z$1.enum(["draft", "published"]).optional()
      },
      handler: async (args) => {
        const collection = asString(args, "collection");
        const id2 = asString(args, "id");
        if (!collection || !id2)
        ...
        if (!col)
          return fail(`Unknown collection: ${collection}`);
        try {
          const doc = await col.findByID(context, id2, {
            perspective: perspectiveArg(args.perspective)
          });
          if (!doc)
            return fail(`Document not found: ${collection}/${id2}`);
          return ok(doc);
        } catch (err) {
    ...
    },
    {
      ...
      inputSchema: {
        collection: z$1.string().describe("Collection name"),
        data: z$1.record(z$1.string(), z$1.unknown()).describe("Field values matching the collection schema"),
        publish: z$1.boolean().optional().describe("Publish immediately (default false)")
      },
      handler: async (args) => {
        const collection = asString(args, "collection");
        const data = asRecord(args, "data");
    ...
    },
    {
      ...
      inputSchema: {
        collection: z$1.string().describe("Collection name"),
        id: z$1.string().describe("Document id"),
        data: z$1.record(z$1.string(), z$1.unknown()).describe("Partial field values to update"),
        publish: z$1.boolean().optional().describe("Publish after updating (default false)")
      },
      handler: async (args) => {
        const collection = asString(args, "collection");
        const id2 = asString(args, "id");
        const data = asRecord(args, "data");
        if (!collection || !id2 || !data)
          return fail("Missing required arguments: collection, id (strings), data (object)");
        const col = api.getCollection(collection);
        if (!col)
          return fail(`Unknown collection: ${collection}`);
        try {
          const result = await col.update(context, id2, data, { publish: args.publish === true });
          if (!result)
            return fail(`Document not found: ${collection}/${id2}`);
          return ok(result);
        } catch (err) {
    ...
    },
    {
      ...
      handler: async (args) => {
        ...
        if (!col)
          return fail(`Unknown collection: ${collection}`);
        try {
          const doc = await col.publish(context, id2);
          if (!doc)
            return fail(`Document not found: ${collection}/${id2}`);
          return ok(doc);
        } catch (err) {
    ...
    },
    {
      ...
      inputSchema: {
        collection: z$1.string().describe("Singleton collection name"),
        perspective: z$1.enum(["draft", "published"]).optional()
      },
      handler: async (args) => {
        const collection = asString(args, "collection");
    ...
    },
    {
      ...
      handler: async (args) => {
        ...
        if (!col)
          return fail(`Unknown collection: ${collection}`);
        const id2 = col.getSingletonId(context);
        if (!id2)
    ...
    },
    {
      ...
      inputSchema: {
        search: z$1.string().optional().describe("Filter by filename/text"),
        assetType: z$1.enum(["image", "file"]).optional(),
      ...
      handler: async (args) => {
        ...
        const offset = typeof args.offset === "number" ? args.offset : void 0;
        try {
          const assets = await assetService.findAssets(orgId, { search, assetType, limit: limit2, offset });
  ...
  async handlePostRequest(ctx, parsedBody) {
    try {
      ...
      if (hasRequests) {
        ...
        if (this.#enableJsonResponse) return await new Promise((resolve2) => {
  ...
  */
  ...
  async send(message, options) {
    ...
    if (isJSONRPCResultResponse(message) || isJSONRPCErrorResponse(message)) {
      ...
      const relatedIds = Array.from(this.#requestToStreamMapping.entries()).filter(([, streamId$1]) => this.#streamMapping.get(streamId$1) === response).map(([id2]) => id2);
      if (relatedIds.every((id2) => this.#requestResponseMap.has(id2))) {
        ...
        if (this.#enableJsonResponse) {
          ...
          const responses = relatedIds.map((id2) => this.#requestResponseMap.get(id2));
          response.ctx.json?.(responses.length === 1 ? responses[0] : responses);
          return;
        } else response.stream?.abort();
        for (const id2 of relatedIds) {
          this.#requestResponseMap.delete(id2);
          this.#requestToStreamMapping.delete(id2);
        }
      }
    }
  }
